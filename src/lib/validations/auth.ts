...
export const userQuerySchema = z.object({
  ...
  sort_by: z.enum(['created_at', 'updated_at', 'email', 'first_name', 'last_name']).optional(),
...
export const updateApiKeySchema = z.object({
  name: z.string().min(1, 'API key name is required').max(100, 'Name is too long').optional(),
  description: z.string().max(500, 'Description is too long').optional(),
  is_active: z.boolean().optional(),
}).refine(
  (data) => Object.keys(data).length > 0,
...
export const validateLoginData = (data: unknown): ValidationResult<z.infer<typeof loginSchema>> => {
  return validateData(loginSchema, data);
};

export const validateRegisterData = (data: unknown): ValidationResult<z.infer<typeof registerSchema>> => {
  return validateData(registerSchema, data);
};

export const validatePasswordChange = (data: unknown): ValidationResult<z.infer<typeof changePasswordSchema>> => {
  return validateData(changePasswordSchema, data);
};

export const validateProfileUpdate = (data: unknown): ValidationResult<z.infer<typeof updateProfileSchema>> => {
  return validateData(updateProfileSchema, data);
};

export const validateUserCreation = (data: unknown): ValidationResult<z.infer<typeof createUserSchema>> => {
