  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const response = await apiClient.get<{ data: DashboardStats }>(`/dashboard/stats?t=${timestamp}`);
        ...
        const validatedStats: DashboardStats = {
          totalPermissions: typeof stats.totalPermissions === 'number' ? stats.totalPermissions : 0,
          totalRoles: typeof stats.totalRoles === 'number' ? stats.totalRoles : 0,
          totalAssociations: typeof stats.totalAssociations === 'number' ? stats.totalAssociations : 0,
          recentActivity: Array.isArray(stats.recentActivity) ? stats.recentActivity.filter(activity => 
            activity && 
